...
% layers, architecture being a n x 1 vector of layer sizes e.g. [784 100 10]
    ...
    nn.size   = architecture;  %nnµÄ½á¹¹:784*100*10
    nn.n      = numel(nn.size);  %3²ã
    
    nn.activation_function              = 'sigm';   %  Activation functions of hidden layers: 'sigm' (sigmoid) or 'tanh_opt' (optimal tanh).
    nn.learningRate                     = 0.002;            %  learning rate Note: typically needs to be lower when using 'sigm' activation function and non-normalized inputs.
    ...
    nn.output                           = 'tanh';       %  output unit 'sigm' (=logistic), 'softmax' and 'linear'
